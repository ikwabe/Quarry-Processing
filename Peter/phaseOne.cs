...
namespace Peter
{
    public partial class phaseOne : UserControl
    {
        public phaseOne()
        {
            InitializeComponent();
        ...
        //new Added
        double benchHeight = 0.00;
        double subdrillLength = 0.00;
        double powderFactor = 0.00;
        double powderFactor2 = 0.00;
        double steamingLength = 0.00;
        ...
        public static double energy_factor = 0.00;
        ...
        void Calculate()
        {
            ...
            if (TotalCost == 0.00 || phaseTwo.TotalCost == 0.00)
            {
                CostDifferenceLabel.Text = "Loading...";
            }
            else
            {
                CostDifference = phaseTwo.TotalCost - TotalCost;
                CostDifferenceLabel.Text = string.Format("{0:n}", CostDifference) + "$";
            }

            if (phaseTwo.TotalUnitCost == 0.00 || TotalUnitCost == 0.00)
            {
                DifferenceLabel.Text = "Loading...";
            }
            else
            {
                TotalUnitDifference = phaseTwo.TotalUnitCost - TotalUnitCost;
        ...
        private void Timer1_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (diameterOfHole > 0.00)
                {
                    ...
                    //updates
                    subdrillLength = diameterOfHole * 9; //mm
                    steamingLength = diameterOfHole * 20;//mm
                    benchHeight = Math.Round(double.Parse(Diameter_of_the_hole.Text) / 15, 1);

                    //updates
                    hole_length = Math.Round(benchHeight + subdrillLength, 1);
                    hole_length_label.Text = hole_length.ToString() + "m";

                    vertical_energy_distribution = Math.Round((((benchHeight - steamingLength) / benchHeight) * 100), 2);
                    energyDistLabel.Text = vertical_energy_distribution.ToString() + "%";


                    burden = diameterOfHole * 25;

                    burdenStiffnessRatio = Math.Round(benchHeight / burden, 1);

                    benchHeightLabel.Text = benchHeight.ToString() + "m";
                    subdrillLenghtLabel.Text = subdrillLength.ToString() + "m";
                    steamingLengthLabel.Text = steamingLength.ToString() + "m";

                    charge_length = Math.Round(hole_length - steamingLength, 1);
                    ...
                    // burden = steamingLength / 0.7;
                    spacing = burden * 1.2;


                    burden_label.Text = burden.ToString() + "m";
                    spacing_label.Text = spacing.ToString() + "m";
            ...
            if (benchHeight == 0.00 || burden == 0.00 || spacing == 0.00 )
            {
            ...
            else
            {
                try
                {
                    volume_of_material_blasted = Math.Round(burden * spacing * benchHeight,2);
                    total_volume_of_material_blasted = Math.Round(drilling_and_blasting_area * benchHeight, 2);
            ...
            //calculate the repeatitive, blasting and available area
            ...
            else
            {
                ...
                benchHeightLabel.Text = "Loading...";
                subdrillLenghtLabel.Text = "Loading...";
                steamingLengthLabel.Text = "Loading...";
                hole_length_label.Text = "Loading...";
                charge_lengh_label.Text = "Loading...";
                spacing_label.Text = "Loading...";
                burden_label.Text = "Loading...";
        ...
        private void PhaseOne_Load(object sender, EventArgs e)
        {
            timer1.Start();
            activityTimer.Start();
            ...
            if (check == false)
            {
                using (SaveFileDialog save = new SaveFileDialog() { Filter = "Excel Workbook|.xlsx" })
                {
                    ...
                    if (save.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        ws.Cells[13, col] = burden;
                        ws.Cells[14, col] = spacing;
                        ws.Cells[15, col] = steamingLength;
                        ws.Cells[16, col] = subdrillLength;
                        ws.Cells[17, col] = benchHeight;
        ...
        private void DarkTheme()
        {
            ...
            //for pannels
            foreach (Control crl in panel4.Controls)
            {
